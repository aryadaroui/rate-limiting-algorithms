...
function experiment(rate_limiter: Function, reqs_per_sec: number, threshold: number, duration: number) {
	...
	let data = {
		experiment: { rate_limiter: rate_limiter.name, reqs_per_sec: reqs_per_sec, threshold: threshold, duration: duration },
		plot: [],
	};
	...
	requester.on('message', async (msg) => {
		...
		let output = rate_limiter('global', threshold, cache);
...
experiment(discrete_window, REQS_PER_SEC, RATE_LIMIT_THRESHOLD, DURATION);
